...
@group(0) @binding(5) var waveVectorTexture: texture_2d<f32>;

fn getDistortionVector(uv: vec2f) -> vec2f {
	let texUv = convertTexCoords(uv);
	let pointerTrailVector = textureSample(pointerTrailTexture, sourceSampler, texUv).rg;
	let waveVector = textureSample(waveVectorTexture, sourceSampler, texUv).rg;
	return waveVector + (pointerTrailVector * uniforms.pointerTrailWeight);
}
...
@group(1) @binding(6) var waveVectorTextureForBlur: texture_2d<f32>;
...
fn getBlurRadius(_uv: vec2f) -> f32 {
	...
	let texUv = convertTexCoords(_uv);
	let pointerTrailVector = textureSample(pointerTrailTextureForBlur, targetSampler, texUv).rg;
	let waveVector = textureSample(waveVectorTextureForBlur, targetSampler, texUv).rg;
	let distortionVector = waveVector + (pointerTrailVector * blurCommonUniforms.pointerTrailWeight);
	...
	let pointerForce = (abs(distortionVector.x) + abs(distortionVector.y)) / 4.0;
