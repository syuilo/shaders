...
fn fsBlur(fragData: VertexOut) -> @location(0) vec4f {
	...
	if (blurUniforms.monteCarlo == 1) {
		...
		for (var i = 0; i < sampleCount; i++) {
			...
			result += select(textureSample(targetTexture, targetSampler, convertTexCoordsClamp(vec2f(fragData.uv.x + (x * r), fragData.uv.y + (y * r)))), textureSampleLevel(targetTexture, targetSampler, convertTexCoordsClamp(vec2f(fragData.uv.x + (x * r), fragData.uv.y + (y * r))), r * 32.0), blurUniforms.test == 1);
		}
		return result / f32(sampleCount);
		//return textureSampleLevel(targetTexture, targetSampler, convertTexCoordsClamp(vec2f(fragData.uv.x, fragData.uv.y)), r * 16.0);
	} else {
		var result = vec4f(0.0);
		...
		for (var i = 0; i < sampleCount; i++) {
			...
			result += select(textureSample(targetTexture, targetSampler, convertTexCoords(sampleUv)), textureSampleLevel(targetTexture, targetSampler, convertTexCoords(sampleUv), r * 32.0), blurUniforms.test == 1) * weight;
