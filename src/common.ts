...
export function createEmptyTextureArray(gl) {
	const texture = gl.createTexture();
...
export async function createTextureFromUrl(gl, url) {
	const texture = gl.createTexture();
...
export async function createTextureArrayFromUrls(gl, width, height, urls) {
	const texture = gl.createTexture();
...
export function makeGl(canvas, vertexShader, fragmentShader, fps = null) {
...
// -1.0 ~ +1.0
float lowqNoise(vec3 v) {
	float a = sin(v.x * 7.0) * sin(v.y * 7.0) * sin(v.z * 10.0);
	float b = sin(v.x * 11.0) * sin(v.y * 13.0) * sin(v.z * 17.0);
	float c = sin(v.x / 3.0) * sin(v.y / 2.5) * sin(v.z / 2.2);
	return (a + b + c) / 3.0;
}

// +0.0 ~ +1.0
float lowqNoise0to1(vec3 v) {
	return (lowqNoise(v) + 1.0) / 2.0;
}
`;
