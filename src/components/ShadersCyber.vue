...
import { createTextureArrayFromUrls, getUrlParam, GLSL_COMMON, makeGl } from '@/common.ts';
...
void main() {
	float time = float(u_time) * u_timeFactor;
	...
	vec2 scroll = vec2(0.0, -time * 0.0001);
	...
	float cellMultiplier2Noise = snoise0to1(vec3(cellUv2.x * 3.0, cellUv2.y * 3.0, time * 0.000025));
	float cellMultiplier4Noise = snoise0to1(vec3(cellUv4.x * 3.0, cellUv4.y * 3.0, time * 0.000025));
	...
	float texSelector = snoise0to1(vec3((cellUv * 3.0) + scroll, time * 0.00001));
	if (ripple > 0.5) {
		texSelector += 0.25;
		//texSelector *= 1.5;
	...
	float scaleNoise = snoise0to1(vec3(cellUv * 0.7, time * 0.0000125));
	...
	float visibilityNoiseA = snoise0to1(vec3(cellUv + scroll, time * 0.00000625));
	float visibilityNoiseB = snoise0to1(vec3(cellUv * 8.0, time * 0.00000625));
	...
	float colorNoise = snoise0to1(vec3((cellUv * 8.0) + scroll, time * 0.000025));
...
onMounted(() => {
	...
	const { gl, program, render } = makeGl(canvas.value, VERTEX_SHADER, FRAGMENT_SHADER, 30);

	const u_seed = gl.getUniformLocation(program, 'u_seed');
	const u_divisions = gl.getUniformLocation(program, 'u_divisions');
	const u_timeFactor = gl.getUniformLocation(program, 'u_timeFactor');
	gl.uniform1f(u_seed, 1000);
	gl.uniform1f(u_divisions, getUrlParam('divisions', 'int') ?? 40);
	gl.uniform1f(u_timeFactor, getUrlParam('timeFactor', 'float') ?? 1.0);
